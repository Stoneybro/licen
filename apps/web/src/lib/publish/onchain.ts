...
export function getDataPolicyAddress(): Address {
  const address = process.env.NEXT_PUBLIC_OG_DATA_POLICY_ADDRESS;
  if (!address) {
    throw new Error("Missing required env var: NEXT_PUBLIC_OG_DATA_POLICY_ADDRESS");
  }
...
export function buildRegisterDatasetArgs(input: {
  ...
  return [
    input.datasetRoot,
    input.manifestHash,
    parseUnits(input.policy.royaltyPerEpoch.toFixed(18), 18),
