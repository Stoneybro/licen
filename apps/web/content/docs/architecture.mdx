...
- policy version (owner consent is implicitly verified by the on-chain registration transaction).
...
## 9.1 Phase 1: Configuration & Encryption (Client-Side)

1. Publisher configures legal terms and dataset metadata via a standard web form (which compiles into a JSON manifest behind the scenes).
2. Dataset is encrypted client-side.
3. Generate `datasetRoot` (hash of ciphertext) and `manifestHash`.

## 9.2 Phase 2: Batched Upload & On-Chain Registration

1. Encrypted blob is uploaded to 0G Storage.
2. Policy manifest JSON is uploaded to 0G Storage.
3. Publisher signs a single blockchain transaction to register the `datasetRoot` and `manifestHash` on the `DataPolicy` contract. This transaction acts as their cryptographic consent to the manifest terms.
