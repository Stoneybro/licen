...
type EnvioJob = {
  ...
  dataset: { manifestHash: string } | null;
};

async function fetchGrantedJobs(): Promise<EnvioJob[]> {
  ...
  const body = (await res.json()) as { data?: { Job?: EnvioJob[] } };
...
async function poll(): Promise<void> {
  ...
  for (const job of jobs) {
    ...
    if (!encryptedKeyEnvelope) {
      console.warn(
        `[poller] No key envelope found for dataset ${job.datasetRoot} (job ${job.id}). ` +
...
export function startPoller(): void {
  console.log(
    `[poller] Starting orchestrator poller (interval: ${POLL_INTERVAL_MS}ms)`
  );
